...
class SQLPipeline:
    ...
    def parse_arguments(self):
        ...
        parser.add_argument('--debug', action='store_true',
                          help='Enable debug mode with detailed path information')
        
        return parser.parse_args()

    def print_debug_info(self):
        """Print detailed debug information about paths and environment"""
        logger.info("=" * 80)
        ...
        logger.info(f"Provided sql-dir argument: {self.args.sql_dir}")
        logger.info(f"Absolute sql-dir path: {os.path.abspath(self.args.sql_dir)}")
        logger.info(f"SQL dir exists: {os.path.exists(self.args.sql_dir)}")
        
        if os.path.exists(self.args.sql_dir):
            logger.info(f"\nContents of {self.args.sql_dir}:")
            try:
                for item in os.listdir(self.args.sql_dir):
                    item_path = os.path.join(self.args.sql_dir, item)
                    item_type = "DIR" if os.path.isdir(item_path) else "FILE"
                    logger.info(f"  [{item_type}] {item}")
                    
                    # Check subdirectories
                    if item in ['Catalogo', 'Bulk-Load'] and os.path.isdir(item_path):
    ...
    def connect_postgres(self) -> Optional[psycopg2.extensions.connection]:
        ...
        logger.info(f"Attempting to connect to PostgreSQL at {self.args.host}:{self.args.port}")
        logger.info(f"Database: {self.args.db_name}, User: {self.args.user}")
        ...
        for attempt in range(self.args.max_retries):
            try:
                ...
                logger.info(f"✓ Connected to PostgreSQL successfully (attempt {attempt + 1})")
                return conn
    ...
    def execute_sql_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            self.conn.commit()
            logger.info(f"✓ Successfully executed {len(statements)} statements from {os.path.basename(file_path)}")
            return True
    ...
    def find_sql_directory(self) -> Optional[str]:
        ...
        logger.info("Searching for SQL directory...")
        ...
        if os.path.exists(self.args.sql_dir):
            ...
            logger.info(f"Checking provided directory: {sql_dir}")
            logger.info(f"  Catalogo exists: {os.path.isdir(catalog_path)}")
            logger.info(f"  Bulk-Load exists: {os.path.isdir(bulk_path)}")

            if os.path.isdir(catalog_path):
                # Check for at least one standard SQL file
                found_files = [f for f in self.standard_sql_files if os.path.exists(os.path.join(catalog_path, f))]
                logger.info(f"  Found {len(found_files)} standard SQL files in Catalogo")
                if found_files:
                    logger.info(f"✓ Using SQL directory: {sql_dir}")
                    return sql_dir
        else:
            logger.warning(f"Provided SQL directory does not exist: {self.args.sql_dir}")
        
        # 2. Check standard locations
        logger.info("Checking standard locations...")
        possible_locations = [
        ...
        for i, location in enumerate(possible_locations, 1):
            ...
            logger.info(f"  [{i}] Checking: {norm_path}")
            if os.path.exists(norm_path):
                catalog_path = os.path.join(norm_path, 'Catalogo')
                if os.path.isdir(catalog_path):
                    found_files = [f for f in self.standard_sql_files if os.path.exists(os.path.join(catalog_path, f))]
                    if found_files:
                        logger.info(f"✓ Found SQL directory: {norm_path}")
                        return norm_path
        
        logger.error("✗ Could not find SQL directory in any location")
        return None
    ...
    def run(self):
        ...
        if not sql_dir:
            ...
            logger.error("1. Verify the --sql-dir path is correct")
            logger.error("2. Ensure the directory contains 'Catalogo' and 'Bulk-Load' subdirectories")
            logger.error("3. Run with --debug flag for more information")
            sys.exit(1)
        ...
        try:
            ...
            if not self.schema_exists(self.args.schema_name):
                logger.error(f"Schema '{self.args.schema_name}' does not exist in database '{self.args.db_name}'")
                logger.error("Please create the schema first or check the schema name")
                sys.exit(1)
            ...
            catalog_dir = os.path.join(sql_dir, 'Catalogo')
            logger.info(f"\nLooking for standard SQL files in: {catalog_dir}")
            for sql_file in self.standard_sql_files:
                file_path = os.path.join(catalog_dir, sql_file)
                if os.path.exists(file_path):
                    all_files.append((sql_file, file_path))
                    logger.info(f"  ✓ Found: {sql_file}")
                else:
                    logger.warning(f"  ✗ Missing: {sql_file}")
            ...
            if os.path.exists(bulk_load_dir):
                logger.info(f"\nLooking for bulk load SQL files in: {bulk_load_dir}")
                for sql_file in self.bulk_load_sql_files:
                    file_path = os.path.join(bulk_load_dir, sql_file)
                    if os.path.exists(file_path):
                        all_files.append((sql_file, file_path))
                        logger.info(f"  ✓ Found: {sql_file}")
                    else:
            ...
            if not all_files:
                logger.error("No SQL files found to process!")
                sys.exit(1)
                
            logger.info(f"\n{'=' * 80}")
            logger.info(f"Ready to process {len(all_files)} SQL files")
            logger.info(f"{'=' * 80}\n")
                
            # 5. Execute files with progress bar
            success_count = 0
            with tqdm(all_files, desc="Processing SQL files", unit="file") as pbar_files:
                for sql_file_name, file_path in pbar_files:
                    pbar_files.set_description(f"Processing {sql_file_name[:30]}")
                    
                    if self.execute_sql_file(file_path):
                        success_count += 1
                        pbar_files.set_postfix(status="✓ OK", file=sql_file_name[:20])
                    else:
                        logger.error(f"✗ Failed to execute {sql_file_name}")
                        pbar_files.set_postfix(status="✗ FAILED", file=sql_file_name[:20])
                        break
            ...
            logger.info(f"Success: {success_count}/{len(all_files)} files processed")
            logger.info(f"{'=' * 80}")
